#!/usr/bin/bash
# Changes the volume of the default sink
# Warning: This script offers no cap for volume,
# I'd advise to not go above 150% (which is the standard cap).
...
function send_notification {
    ...
    dunstify "Volume: $overvolume%" \
        -h int:value:$volume \
        -i /usr/share/icons/Adwaita/96x96/status/audio-volume-medium-symbolic.symbolic.png \
        -t 1000 --replace=555 -u normal
}

case $1 in
    up)
        # Unmute (if muted) then raise volume to the next multiple of 5
        ...
        ;;
    down)
        # Unmute (if muted) then lower volume to the previous multiple of 5
        pactl set-sink-mute @DEFAULT_SINK@ 0 > /dev/null
        curvol=`get_volume`
        rem=$(( curvol % 5 ))
        step=$(( rem == 0 ? 5 : rem ))
    ...
    mute)
        ...
        if [[ `is_mute` == "Mute: yes" ]]; then
            dunstify -i "${HOME}/.config/rice_assets/Icons/mute.png" \
                --replace=555 -u normal "Volume: Mute" -t 1000 -u critical
        else
            send_notification
        fi
        ;;
